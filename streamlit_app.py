...
def main():
    ...
    with st.sidebar:
        ...
        allocations = {
            'SPY': st.number_input("S&P 500 (SPY) Allocation %", min_value=0, max_value=100, value=40, key="alloc_SPY", on_change=reset_run_dashboard),
            'QQQ': st.number_input("NASDAQ (QQQ) Allocation %", min_value=0, max_value=100, value=35, key="alloc_QQQ", on_change=reset_run_dashboard),
            'IWM': st.number_input("Russell 2000 (IWM) Allocation %", min_value=0, max_value=100, value=25, key="alloc_IWM", on_change=reset_run_dashboard)
        }
        ...
        if total_allocation != 100:
            ...
            st.stop()
        st.divider()
        start_date_input = st.date_input("Analysis Start Date", datetime.date(2020, 1, 1), key="start_date", on_change=reset_run_dashboard)
        end_date_input = st.date_input("Analysis End Date", datetime.date(2023, 1, 1), key="end_date", on_change=reset_run_dashboard)
    ...
    if st.session_state.run_dashboard:
        ...
        with col1:
            ...
            fig = px.line(cum_returns_df, x='Date', y='Cumulative Return', title="Portfolio Cumulative Returns")
        ...
        fig_corr = px.imshow(corr_matrix, color_continuous_scale='RdBu', range_color=[-1, 1], title="Asset-Factor Correlation Matrix")
        st.plotly_chart(fig_corr, use_container_width=True, key="corr_chart")
        st.header("Additional Portfolio Statistics")
        hist_fig = px.histogram(portfolio_returns, nbins=50, title="Histogram of Daily Returns", labels={'value': 'Daily Return'})
