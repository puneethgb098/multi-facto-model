...
def compute_diagnostics(portfolio_returns, trading_days=252):
    """
    Computes key diagnostic metrics for the portfolio.
    """
...
def run_analysis(allocations, start_date_input, end_date_input):
    ...
    Runs the entire analysis pipeline:
      - Fetches portfolio and factor data
      - Computes weighted portfolio returns based on allocations
      - Performs factor analysis (for demonstration)
      - Computes portfolio diagnostics

    Returns:
        weighted_portfolio_returns (pd.Series): The portfolio's daily returns.
        diagnostics (dict): Key performance metrics.
    """
...
def main():
    ...
    # Initialize session state variable for triggering dashboard run
    ...
    # Sidebar: Portfolio Configuration and Date Inputs
    with st.sidebar:
    ...
    # Run analysis and display the dashboard if triggered
